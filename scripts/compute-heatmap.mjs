#!/usr/bin/env node
/**
 * Precompute heatmap data for the historical map.
 *
 * This script samples a grid of points and calculates:
 * - contested: number of distinct civilizations that controlled each point
 * - settled: total years each point was under any civilization
 *
 * Run with: node scripts/compute-heatmap.js
 * Output: data/heatmap.json
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Grid sampling configuration
const GRID_RESOLUTION = 1; // degrees
const LAT_MIN = -60;
const LAT_MAX = 75;
const LON_MIN = -180;
const LON_MAX = 180;

// Point-in-polygon using ray casting algorithm
function pointInPolygon(point, polygon) {
    const x = point[0], y = point[1];
    let inside = false;

    for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
        const xi = polygon[i][0], yi = polygon[i][1];
        const xj = polygon[j][0], yj = polygon[j][1];

        if (((yi > y) !== (yj > y)) && (x < (xj - xi) * (y - yi) / (yj - yi) + xi)) {
            inside = !inside;
        }
    }
    return inside;
}

function pointInGeometry(lon, lat, geometry) {
    const point = [lon, lat];

    if (geometry.type === 'Polygon') {
        return pointInPolygon(point, geometry.coordinates[0]);
    } else if (geometry.type === 'MultiPolygon') {
        for (const poly of geometry.coordinates) {
            if (pointInPolygon(point, poly[0])) {
                return true;
            }
        }
    }
    return false;
}

// Compute bounding box for a geometry
function getBoundingBox(geometry) {
    let minLon = Infinity, maxLon = -Infinity;
    let minLat = Infinity, maxLat = -Infinity;

    function processCoords(coords) {
        for (const coord of coords) {
            if (Array.isArray(coord[0])) {
                processCoords(coord);
            } else {
                minLon = Math.min(minLon, coord[0]);
                maxLon = Math.max(maxLon, coord[0]);
                minLat = Math.min(minLat, coord[1]);
                maxLat = Math.max(maxLat, coord[1]);
            }
        }
    }

    processCoords(geometry.coordinates);
    return { minLon, maxLon, minLat, maxLat };
}

function pointInBoundingBox(lon, lat, bbox) {
    return lon >= bbox.minLon && lon <= bbox.maxLon &&
           lat >= bbox.minLat && lat <= bbox.maxLat;
}

async function main() {
    const dataDir = path.join(__dirname, '..', 'data');
    const politiesPath = path.join(dataDir, 'cliopatria_polities_only.geojson');
    const outputPath = path.join(dataDir, 'heatmap.json');

    console.log('Loading polities data...');
    const politiesData = JSON.parse(fs.readFileSync(politiesPath, 'utf8'));
    const polities = politiesData.features;
    console.log(`Loaded ${polities.length} polities`);

    // Pre-compute bounding boxes
    console.log('Computing bounding boxes...');
    const polityBboxes = polities.map(p => ({
        polity: p,
        bbox: getBoundingBox(p.geometry)
    }));

    const contestedData = [];
    const settledData = [];

    let maxContested = 0;
    let maxSettled = 0;

    const totalLats = Math.ceil((LAT_MAX - LAT_MIN) / GRID_RESOLUTION);
    const totalLons = Math.ceil((LON_MAX - LON_MIN) / GRID_RESOLUTION);
    const totalPoints = totalLats * totalLons;

    console.log(`Sampling ${totalPoints} grid points (${totalLats} x ${totalLons})...`);

    let processedPoints = 0;
    let lastProgress = 0;
    const startTime = Date.now();

    for (let lat = LAT_MIN; lat <= LAT_MAX; lat += GRID_RESOLUTION) {
        for (let lon = LON_MIN; lon <= LON_MAX; lon += GRID_RESOLUTION) {
            let contestedCount = 0;
            let settledYears = 0;
            const seenPolities = new Set();

            // Check each polity
            for (const { polity, bbox } of polityBboxes) {
                // Quick bounding box rejection
                if (!pointInBoundingBox(lon, lat, bbox)) continue;

                // Full point-in-polygon test
                if (pointInGeometry(lon, lat, polity.geometry)) {
                    const name = polity.properties.Name;
                    const duration = polity.properties.ToYear - polity.properties.FromYear;

                    // Count distinct polity names for "contested"
                    if (!seenPolities.has(name)) {
                        seenPolities.add(name);
                        contestedCount++;
                    }

                    // Sum total years for "settled"
                    settledYears += duration;
                }
            }

            if (contestedCount > 0) {
                // Store as [lat, lon, value] for Leaflet.heat format
                contestedData.push([lat, lon, contestedCount]);
                settledData.push([lat, lon, settledYears]);
                maxContested = Math.max(maxContested, contestedCount);
                maxSettled = Math.max(maxSettled, settledYears);
            }

            processedPoints++;
            const progress = Math.floor((processedPoints / totalPoints) * 100);
            if (progress > lastProgress && progress % 5 === 0) {
                lastProgress = progress;
                const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
                process.stdout.write(`\rProgress: ${progress}% (${elapsed}s elapsed)`);
            }
        }
    }

    const totalTime = ((Date.now() - startTime) / 1000).toFixed(1);
    console.log(`\nCompleted in ${totalTime}s`);
    console.log(`Found ${contestedData.length} data points with civilization presence`);
    console.log(`Max contested: ${maxContested} civilizations`);
    console.log(`Max settled: ${maxSettled} years`);

    // Save to JSON
    const output = {
        metadata: {
            gridResolution: GRID_RESOLUTION,
            bounds: { latMin: LAT_MIN, latMax: LAT_MAX, lonMin: LON_MIN, lonMax: LON_MAX },
            dataPoints: contestedData.length,
            maxContested,
            maxSettled,
            computedAt: new Date().toISOString()
        },
        contested: contestedData,
        settled: settledData
    };

    console.log('Writing output...');
    fs.writeFileSync(outputPath, JSON.stringify(output));

    const fileSize = (fs.statSync(outputPath).size / 1024 / 1024).toFixed(2);
    console.log(`Saved to ${outputPath} (${fileSize} MB)`);
}

main().catch(console.error);
